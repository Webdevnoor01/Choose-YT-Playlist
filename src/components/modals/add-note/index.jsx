...
const AddNote = ({ open, scroll, handleClickOpen, handleClose }) => {
  ...
  // useForm hook 
  const { formState:{errors}, setValue, handleSubmit, control } = useForm({
    defaultValues: {
      noteData: "",
    },
  ...
  const currentVideo = useSelector(
    (state) => state.playlist.items[playlistId].videos[index].videoThumbnail
  );
  const channleTitle = useSelector(state =>state.playlist.items[playlistId].channleTitle)
  ...
  const onInValid = (errors) => {
    ...
    errorKeys.map((key) => showToast({type:"error", message:errors[key].message}))
  };
